...
import type { KeywordResult, AISuggestions } from "@/types";
...
export function KeywordsPanel() {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 max-w-5xl mx-auto w-full">
      ...
      {error && (
        <div className="mb-6 p-4 bg-red-900/30 border border-red-800 rounded-lg text-red-300 text-sm">{error}</div>
      )}
          ...
          <section>
            ...
            <div className="flex flex-wrap gap-2">
              {result.titleKeywords.slice(0, 30).map(({ word, count }, i) => (
                <span key={i} className="px-3 py-1 bg-gray-900 text-sm rounded-lg"
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {aiSuggestions.keywords.map((kw, i) => (
                    <span key={i} className="px-3 py-1 bg-orange-900/20 border border-orange-800/30 text-orange-300 text-sm rounded-lg">{kw}</span>
                  ))}
                </div>
              </div>
            ) : !aiLoading ? (
